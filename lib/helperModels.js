...
class BankAmount {
   ...
   * @param {string} dcmark - D or C = Debit or credit (C = positive); prefix 'R' -> change of sign, reversed; prefix 'E' -> expected entry (sign unchanged)
  ...
  static parse(dcmark, amountStr) {
    ...
    if (mark && mark !== 'R' && mark !== 'E') {throw Error( `Wrong reversal mark: ${dcmark}` );}
    ...
    if (mark === 'R') {amount = amount.multipliedBy(-1);} // Reversal flips the sign; Expected ('E') keeps it
