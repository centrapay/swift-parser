...
class StatementVisitor {
  ...
  constructor(opts) {
    ...
    this.messageBlocks = {};
    this.transactions = [];
    this.informationToAccountOwnerLines = [];
    this.tags = [];
  }
  ...
  toStatement() {
    ...
    if (this.informationToAccountOwnerLines.length) {
      statement.informationToAccountOwner = this.informationToAccountOwnerLines.join('\n');
    }
    if (Object.keys(this.messageBlocks).length) {
      statement.messageBlocks = this.messageBlocks;
    }
  ...
  visitTransactionDetails(tag) {
    if (this.prevTag instanceof Tags.TagStatementLine) {
      ...
      this.informationToAccountOwnerLines.push(tag.fields.transactionDetails);
    }
    this.pushTag(tag);
  }
...
class Parser {
  ...
  */
  parse({ data, with86Structure = true, withTags = false }) {
    ...
    const statements = tagGroups.map((group, idx) => {
      this._validateGroup(group, idx+1);
      return this._buildStatement({ group, withTags, with86Structure });
    });
  ...
  * @private
  */
  _buildStatement({ group, withTags, with86Structure }) {
